...
class PiConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        await self.accept()
        logger_main.debug(f"WebSocket: Raspberry Pi connected: {self.channel_name}")
    ...
    async def disconnect(self, close_code):
        logger_main.debug(f"WebSocket: Raspberry Pi disconnected: {self.channel_name} with code: {close_code}")
    ...
    async def receive(self, text_data=None, bytes_data=None):
        if text_data:
            ...
            # The Process the received text message
            logger_main.debug(f"Received text message: {message}")
            await self.send(text_data=json.dumps({
            ...
            logger_main.debug(f"Received binary data: {bytes_data}")
